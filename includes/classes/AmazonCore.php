...
abstract class AmazonCore
{
     ...
     */
    protected function __construct($s = null, $mock = false, $m = null, $config = [])
    {
...
<ErrorResponse xmlns="http://mws.amazonaws.com/doc/2009-01-01/">
  ...
  <RequestID>123</RequestID>
    ...
    public function setConfig($config)
    {
        if (!empty($config)) {
            $this->config = $config;
            if ($logpath = array_get($config, 'logpath')) {
                $this->setLogPath($logpath);
            } else {
                throw new Exception("Logpath is empty!");
            }

            if ($AMAZON_SERVICE_URL  = array_get($config, 'AMAZON_SERVICE_URL')) {
    ...
    public function setStore($s = null)
    {
        ...
        if (!$store = array_get($config, 'store')) {
    ...
    protected function log($msg, $level = 'Info')
    {
        if ($msg != false) {
            ...
            if ($logfunction = array_get($config, 'logfunction') && function_exists($logfunction)) {
                switch ($level) {
                    case ('Info'):
                        $loglevel = LOG_INFO;
                        break;
